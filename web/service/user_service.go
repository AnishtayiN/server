...
func generateVMessLink(client *database.Client, inbound *database.Inbound, domain string, isTLS bool) string {
	...
	if inbound.StreamSettings != "" {
		// Parse stream settings to get network type and other configs
		// Simplified for now
		network = "tcp"
	}
...
func GenerateSubscriptionLink(client *database.Client, domain string, isTLS bool) string {
	...
	// Get all enabled inbounds for this client's protocols
	var inbounds []database.Inbound
...
func GetStats() (map[string]interface{}, error) {
	...
	db.Model(&database.Inbound{}).Select("SUM(total_traffic)").Scan(&totalTraffic)
	db.Model(&database.Inbound{}).Select("SUM(used_traffic)").Scan(&usedTraffic)
